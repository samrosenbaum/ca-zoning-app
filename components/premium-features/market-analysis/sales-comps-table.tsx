...
import type { SalesComp } from "@/lib/types"

interface SalesCompsTableProps {
  comps: SalesComp[]
}

export default function SalesCompsTable({ comps }: SalesCompsTableProps) {
  const [sortField, setSortField] = useState<keyof SalesComp>("pricePerSqFt")
  const [sortDirection, setSortDirection] = useState<"asc" | "desc">("desc")

  const handleSort = (field: keyof SalesComp) => {
    if (field === sortField) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <h3 className="text-lg font-medium">Sales Comparables</h3>
        <DropdownMenu>
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead>
                <Button variant="ghost" onClick={() => handleSort("salePrice")} className="p-0 h-auto font-medium">
                  Sale Price
                  <ArrowUpDown className="ml-2 h-3 w-3" />
                </Button>
              </TableHead>
              ...
              <TableHead>
                <div className="flex items-center">
                  <Button variant="ghost" onClick={() => handleSort("capRate")} className="p-0 h-auto font-medium">
                    Cap Rate
                    <ArrowUpDown className="ml-2 h-3 w-3" />
                  </Button>
                  <TooltipProvider>
                    <Tooltip>
                      ...
                      <TooltipContent>
                        <p className="w-[200px] text-xs">Net operating income divided by property value</p>
                      </TooltipContent>
                    </Tooltip>
                  </TooltipProvider>
                </div>
              ...
              <TableHead>
                <Button variant="ghost" onClick={() => handleSort("saleDate")} className="p-0 h-auto font-medium">
                  Sale Date
                  <ArrowUpDown className="ml-2 h-3 w-3" />
                </Button>
              </TableHead>
          ...
          <TableBody>
            {sortedComps.map((comp) => (
              <TableRow key={comp.id}>
                ...
                <TableCell>{comp.capRate ? `${comp.capRate.toFixed(2)}%` : "N/A"}</TableCell>
      ...
      <div className="text-sm text-gray-500 italic">
        {comps.length} sales comparables found within {comps.length > 0 ? comps[0].distanceFromTarget.toFixed(1) : "0"}{" "}
        miles
      </div>
    </div>
  )
